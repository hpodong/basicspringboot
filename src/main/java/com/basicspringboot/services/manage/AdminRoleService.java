...
public class AdminRoleService extends _BSService<AdminRole> {
    ...
    public Long insert(AdminRole data) {
        return super.insertReturnKey(AdminRole.class, setter -> setter.putAll(data.toInput()));
    }
    ...
    public void refresh() {
        ...
        roles.clear();
        roles.addAll(super.jt.query(bsq.toSql(), (rs, rn) -> new AdminRole(rs, rn)));
    }
    ...
    public List<Long> findItemsIdx(Long ar_idx) {
        final String sql  = """
                ...
                WHERE ar_idx = ?
                """;
        return jt.query(sql, (rs, rn) -> rs.getLong("am_idx"), ar_idx);
    }
}