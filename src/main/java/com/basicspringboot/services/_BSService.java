...
import com.basicspringboot.exceptions.NotFoundDeletedAtColumnException;
import com.basicspringboot.exceptions.NotFoundIdxException;
import com.basicspringboot.exceptions.NotFoundWhereException;
import com.basicspringboot.interfaces.AdminListViewSetter;
...
import com.basicspringboot.models.AdminListview;
...
@Service
@Slf4j
public abstract class _BSService<T extends _BSModel>{
    ...
    public List<T> findAll(BSQuery bsq, RowMapper<T> rm, Object ...objects) {
        bsq.addArgs(objects);
        return jt.query(bsq.toSql(), bsq.toJDBCTemplate(), rm);
    }

    public long findAllCount(BSQuery bsq, Object ...objects) {
        bsq.addArgs(objects);
        return jt.queryForObject(bsq.toCountSql(), bsq.toJDBCTemplate(), (rs, rn) -> rs.getLong(1));
    }

    @Transactional
    public AdminListview<T> findAllListView(BSQuery bsq, Long count, AdminListViewSetter<T> setter, Object ... objects) {
        bsq.addArgs(objects);
    ...
    @Transactional
    public AdminListview<T> findAllListView(BSQuery bsq, AdminListViewSetter<T> setter, Object ... objects) {
        bsq.addArgs(objects);
        final long count = findAllCount(bsq, objects);
        final List<T> list = this.findAll(bsq, (rs, rn) -> setter.setter(bsq.getOffset(), count, rs, rn));

        return new AdminListview<>(list, count, bsq.getLimit(), bsq.getPage());
    }

    public long findAllVisitedCount(BSQuery bsq) {
        return jt.queryForObject(bsq.toObjectSql("COUNT(*)"), Long.class);
    }

    public long findAllRowCount(BSQuery bsq) {
        final StringBuilder sb = new StringBuilder();
        sb.append("SELECT COUNT(*) FROM (");
        sb.append(bsq.toSql(true));
        sb.append(") as row_count");
        return jt.queryForObject(sb.toString(), Long.class);
    }

    public T findOne(BSQuery bsq, RowMapper<T> rm, Object ... objects) {
        bsq.setLimit(1);
        bsq.addArgs(objects);
        try {
            return jt.queryForObject(bsq.toSql(), bsq.toJDBCTemplate(), rm);
    ...
    protected boolean update(Class<?> clazz, String where, DataSetter setter) {
    ...
    public int deleteFromIdx(Class<? extends _BSModel> clazz, boolean isForce, Object ...idx) {
        ...
        final int result = jt.update(sb.toString(), idx);
    ...
    protected int delete(String table, String where, Object... objs) {
        ...
        final int result = jt.update(sb.toString(), objs);
        if(result == 0) throw new DeleteException();
        return result;
    }
