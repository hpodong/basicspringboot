...
public class NoticeService extends _BSService<Notice> {
    ...
    public Long insert(Notice data) {
        return super.insertReturnKey(Notice.class, setter -> setter.putAll(data.toInput()));
    }
    ...
    public Map<String, Object> getFileMaxSortFromDataIdx(Long idx) {
        final String sql = """
                ...
                WHERE n_idx = ?
                """;
        return jt.queryForObject(sql, (rs, rn) -> {
    ...
    public List<FileModel> getFilesFromDataIdx(Long idx) {
        final String sql = """
                ...
                ORDER BY nf_sort
                """;
        return jt.query(sql, (rs, rn) -> new FileModel(rs, rn), idx);
    }
}
