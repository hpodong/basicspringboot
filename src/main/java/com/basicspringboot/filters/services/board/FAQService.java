package com.basicspringboot.filters.services.board;

import com.basicspringboot.filters.dto.BSQuery;
import com.basicspringboot.filters.models.board.FAQ;
import com.basicspringboot.filters.models.board.FAQCategory;
...
public class FAQService extends _BSService<FAQ> {
    ...
    private void updateSort(FAQ data) {
        master.update("CALL update_faq_sort(?, ?, ?)", data.getIdx(), data.getCategory_idx(), data.getSort());
    }

    public void refreshCategories() {
        final BSQuery bsq = new BSQuery(FAQCategory.class);
        final List<FAQCategory> categories = super.master.query(bsq.toSql(), (rs, rn) -> new FAQCategory(rs, rn));
